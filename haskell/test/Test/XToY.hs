  ...
  testProperty "swapping within a single tick range" $ property do
    ...
    clevelandProp do
      ...
      for_ swaps \dx -> do
        ...
        let expectedFeeGrowth =
              sFeeGrowth initialSt +
                PerToken (mkX @Natural @128 expectedFee `div` X (fromIntegral @Integer @Natural liquidityDelta)) 0
      ...
      let expectedFees =
            ...
            & sum
      -- The fee might be rounded down, so it's possible 1 X token is lost.
...
test_many_small_swaps =
  nettestScenarioOnEmulatorCaps "placing many small swaps is (mostly) equivalent to placing 1 big swap" do
    ...
    checkCompares (cfmm1YBalance, cfmm1YBalance + swapCount) inRange cfmm2YBalance
...
test_crossing_ticks =
  nettestScenarioOnEmulatorCaps "executing a swap within a single tick range or across many ticks should be (mostly) equivalent" do
    ...
    checkCompares (lowerTickIndex, lowerTickIndex + 50) inRange (sCurTickIndex st1)
    ...
    checkCompares (feeGrowthX1, feeGrowthX1 + marginOfError) inRange feeGrowthX2
    ...
    checkCompares (cfmm1BalanceDeltaY, cfmm1BalanceDeltaY + 10) inRange cfmm2BalanceDeltaY
    ...
    checkCompares (feeReceiver1BalanceX - 10, feeReceiver1BalanceX + 10) inRange feeReceiver2BalanceX
